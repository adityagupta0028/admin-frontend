...
interface AddEarringsProductProps {
  show: boolean;
  handleClose: () => void;
  categories?: any[];
  subCategories?: any[];
...
function AddEarringsProduct({ show, handleClose, categories = [], subCategories = [], onSuccess }: AddEarringsProductProps) {
  // Form state
  ...
  const [dropShapeDropdownOpen, setDropShapeDropdownOpen] = useState(false);
  ...
  const dropShapeDropdownRef = useRef<HTMLDivElement>(null);
  ...
  const unitOfSaleStatic = [
    { id: 1, label: "Pair", value: "pair" },
    { id: 2, label: "Single", value: "single" },
    { id: 3, label: "Set", value: "set" },
  ];

  const dropShapeStatic = [
    { id: 1, label: "Round", value: "round" },
    { id: 2, label: "Oval", value: "oval" },
    { id: 3, label: "Pear", value: "pear" },
    { id: 4, label: "Teardrop", value: "teardrop" },
    { id: 5, label: "Heart", value: "heart" },
    { id: 6, label: "Marquise", value: "marquise" },
    { id: 7, label: "Princess", value: "princess" },
    { id: 8, label: "Cushion", value: "cushion" },
  ];

  const attachmentTypeStatic = [
    { id: 1, label: "Post", value: "post" },
    { id: 2, label: "Hook", value: "hook" },
    { id: 3, label: "Clip", value: "clip" },
    { id: 4, label: "Hoop", value: "hoop" },
    { id: 5, label: "Threader", value: "threader" },
  ];

  const earringOrientationStatic = [
    { id: 1, label: "Dangle", value: "dangle" },
    { id: 2, label: "Drop", value: "drop" },
    { id: 3, label: "Stud", value: "stud" },
    { id: 4, label: "Hoop", value: "hoop" },
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (dropShapeDropdownRef.current && !dropShapeDropdownRef.current.contains(event.target as Node)) {
    ...
    <Modal centered show={show} onHide={handleClose} size="lg" className="modal-parent">
      <Modal.Body className="modal-body-scrollable">
        <div className="custom-form-container">
          <h3 className="mb-4 fw-bold text-center text-black">Add Earrings Product</h3>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="dropdown-multi">
              <div className="mb-3 ddr-width" ref={dropShapeDropdownRef}>
                ...
                <div className={`dropdown ${dropShapeDropdownOpen ? "active" : ""}`}>
                  <div
                    className="dropdown-select"
              ...
              {isCenterStoneAccordionOpen && (
                <div className="mt-3">
                    ...
                    <div className="w-100 half-divide">
                      ...
                      <div className="form-check w-50">
                        <input
                          ...
                          onChange={() => toggleStone("None")}
                        />
                        <label className="form-check-label text-black" htmlFor="cs-stone-none">
                          None
                        </label>
                      </div>
                    </div>
                  ...
                  {/* <div className="mb-3">
                    ...
                    <div className="w-100">
                      {diamondOriginStatic.map((origin) => (
                        <div className="form-check form-check-inline" key={`cs-origin-${origin.id}`}>
                          <input
                            className="form-check-input"
                            type="checkbox"
                            id={`cs-origin-${origin.value}`}
                            value={origin.value}
                            checked={diamondOrigins.includes(origin.value)}
                            onChange={() => toggleDiamondOrigin(origin)}
                          />
                          <label className="form-check-label text-black" htmlFor={`cs-origin-${origin.value}`}>
                            {origin.label}
                          </label>
                        </div>
                      ))}
                  ...
                  {/* 4. Diamond Shape - Radio button options (checkbox style per request description "Radio button options (checkbox)") */}
                  {/* Using Radio buttons as selecting a shape is typically single choice, but description had (checkbox) */}
                  {/* Assuming single selection for shape is best, but using radio input type */}
                  <div className="mb-3">
                  ...
                  {/* 5. Minimum Stone Weight - Text Input */}
                  {/* <div className="mb-3">
                    <label className="form-label text-black">Minimum Stone Weight</label>
                    <input
                      type="text"
                      className="form-control"
                      placeholder="Enter Minimum Weight"
                      value={centerStoneMinWeight}
                      onChange={(e) => setCenterStoneMinWeight(e.target.value)}
                    />
                  </div> */}
...
export default AddEarringsProduct
