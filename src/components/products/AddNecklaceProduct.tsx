...
function AddNecklaceProduct({ show, handleClose, categories = [], subCategories = [], onSuccess }: AddNecklaceProductProps) {
  ...
  // Assembly Types from API
  const assemblyTypes = (assemblyTypesData?.data as any[]) || [];

  // Chain Types from API
  const chainTypes = (chainTypesData?.data as any[]) || [];

  // Closure Types from API
  const closureTypes = (closureTypesData?.data as any[]) || [];

  // Stone Settings from API
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (ornamentDetails.length === 0) {
      toast.error("Please select at least one Placement Fit");
      return;
    }
    ...
    try {
      ...
      // Sizes - For necklaces, only send necklace_size, not ring_size
      // ringSizes.forEach((size) => formData.append("ring_size", size)); // Not needed for necklaces
      ...
      await createNecklaceProduct(formData).unwrap();
      ...
      <Modal.Body className="modal-body-scrollable">
        <div className="custom-form-container">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              <div className="w-100" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {caratWeightOptions.map((weight, index) => {
                  ...
                  const selectedAssembly = assemblyTypes.find(item => item._id === settingConfigurations);
                  const isTrilogyOrToiEtMoi = selectedAssembly?.displayName === "Trilogy Setting" || selectedAssembly?.displayName === "Toi Et Moi Setting";
            ...
            {/* <div className="mb-3">
              <label className="form-label text-black">Size Types</label>
              <div>
                <div className="form-check form-check-inline">
                  <input
                    className="form-check-input"
                  ...
                  chainTypes.map((item) => (
                    <div className="form-check form-check-inline" key={item._id}>
                      <input
                        ...
                        id={`chainType-${item._id}`}
                        value={item._id}
                        checked={shankConfigurations === item._id}
                        onChange={(e) => setShankConfigurations(e.target.value)}
                      />
                      <label className="form-check-label text-black" htmlFor={`chainType-${item._id}`}>
                        {item.displayName}
                      </label>
                    </div>
                  ))
                ) : (
                  <div className="text-muted">Loading chain types...</div>
                )}
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              <div>
                {closureTypes.length > 0 ? (
                  ...
                  <div className="text-muted">Loading closure types...</div>
                )}
              </div>
            </div>
